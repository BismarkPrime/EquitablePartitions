...
def test_speed_parallel(bertha_dict,sizes)
    # Initialize MPI communicator
    comm = MPI.COMM_WORLD
    thread = comm.Get_rank()        # the processor the current code is running on
    total_threads = comm.Get_size()

    # get times if already recorded for certain sizes
    if 'times_parallel.pkl' in os.listdir():
        print("loading previously calculated times...")
        try:
            with open('times_parallel.pkl','rb') as infile:
                time_dict = pickle.load(infile)
                our_times = time_dict['us']
                their_times = time_dict['them']
                sizes_to_skip = time_dict['sizes']
...
python speed_test.py args
    args:
        - 'slurm' to run via sbatch on the supercomputer. If you specify you must include:
            o 'top_power' followed by a number of the highest power of 2 to run the speed test to.
        'fake_parallel' to do a speed run with the fake_parallelization method.
"""
if __name__ == "__main__":
    fake_parallel = False
    prompt_user = True
    try:
        if 'fake_parallel' in sys.argv:
            print("Conducting fake parallelized Speed Test")
            fake_parallel = True
        if 'slurm' in sys.argv:
            prompt_user = False
            graph_fname = 'speed_comp.png'
            top_power2 = int(sys.argv[sys.argv.index('top_power') + 1])
    except:
