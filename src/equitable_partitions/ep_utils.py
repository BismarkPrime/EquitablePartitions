...
def getEigenvaluesSparse(mat: sparse.sparray) -> List[float | complex]:
    ...
    # 2. Find Fundamental LEP Set (Monad LEP Set)
    leps = lep_finder.getLocalEquitablePartitions(lep_finder.initFromSparse(csc), pi)
    print(f"{leps=}")

    globals, locals, _ = _getEigenvaluesSparse(csc, csr, pi, leps)
    # plot the equitable partition if desired
    print(f"{csr.todense()=}")
    plotEquitablePartition(nx.from_scipy_sparse_array(csr, create_using=nx.DiGraph), pi)
...
def plotEquitablePartition(G, pi, pos_dict=None):
    ...
    color = iter(plt.cm.get_cmap("rainbow")(np.linspace(0, 1, len(pi) + 1)))
...
def printStats(G: nx.Graph | nx.DiGraph) -> None:
    ...
    ep, leps = getEquitablePartitions(G)
...
def getReciprocatingEquitablePartitions(
        G: nx.Graph | nx.DiGraph, progress_bars: bool = True
) -> Tuple[Dict[int, List[Any]], List[List[int]]]:
    """Finds the coarsest reciprocating equitable partition and local reciprocating equitable partitions of a graph.
    ...
    RETURNS:
        The reciprocating equitable partition (dict; int -> set), local reciprocating equitable partition (list of sets
            of partition elements grouped together)
    """
    # get receiving  and transmitting eps and leps
    r_ep, r_leps = getEquitablePartitions(G,False)
    t_ep, t_leps = getEquitablePartitions(G,False,rev=True)

    return getREPsFromReceivingTransmitting(r_ep,r_leps,t_ep,t_leps,G)

def getREPsFromReceivingTransmitting(
        r_ep: Dict[int, List[Any]], r_leps: List[List[int]],
        t_ep: Dict[int, List[Any]], t_leps: List[List[int]],
        G: nx.Graph | nx.DiGraph
    ) -> Tuple[Dict[int, List[Any]], List[List[int]]]:
    """Finds the coarsest reciprocating equitable partition and local reciprocating equitable partitions of a graph.
    ...
    ARGUMENTS:
        r_ep, t_ep: dicts
            receiving and transmitting coarsest equitable partition dictionary respectively
        r_leps, t_leps: list of lists
            receiving and transmitting local equitable partition list of lists respectively
        n: int
    ...
    RETURNS:
        The reciprocating equitable partition (dict; int -> set), local reciprocating equitable partition (list of sets
            of partition elements grouped together)
    """
    # list with label of ep element per node in receiving and transmiting ep elements
    # rec_labels = [i for i in r_ep.keys() for _ in range(len(r_ep[i]))]

    trans_labels = [None] * nx.number_of_nodes(G)
    # TODO: this needs to be redone so it keeps the same order as the receiving, otherwise these labels are off.
    for k in t_ep.keys():
        for n in t_ep[k]:
            trans_labels[n] = k
    ...
    # cycle through without dependence on ordering:
    recip_ep = {}
    label_idx = 0

    for r_block in r_ep.values():
        director = {}

        for node in r_block:
            t_label = trans_labels[node]

            if t_label not in director:
                director[t_label] = label_idx
                recip_ep[label_idx] = []
                label_idx += 1

            recip_ep[director[t_label]].append(node)

    # cycle through each node and assign it an equitable partition based on changes
    # for node in range(1, len(rec_labels)):
    #     if rec_labels[node] != rec_labels[node-1]:
    #         director = {trans_labels[node]: label_idx}
    #         recip_ep[label_idx] = [node]
    #         label_idx += 1
    #     else:
    #         if trans_labels[node] not in director.keys():
    #             director[trans_labels[node]] = label_idx
    #             recip_ep[label_idx] = [node]
    #             label_idx += 1
    #         else:
    #             recip_ep[director[trans_labels[node]]].append(node)

        # DEBUGGING PURPOSES: can also be used to illustrate
        # print(node)
        # print(director)
        # print(f"recip_ep: {recip_ep}")

    # this might need to change later but for now I'm seeing if it works with the original receiving leps
    N_G = lep_finder.initFromNx(nx.convert_node_labels_to_integers(G))
    recip_leps = lep_finder.getLocalEquitablePartitions(N_G, recip_ep)

    return recip_ep, recip_leps
...
def GetSpectrumFromLEPs(
    ...
    if partition_data is None:
        if directed: ep_dict, lep_list = getEquitablePartitions(G.reverse(), progress_bars=progress_bars)
        else: ep_dict, lep_list = getEquitablePartitions(G, progress_bars=progress_bars)
    else:
    ...
    if with_grand_divisor:
        ...
        if directed: total_spec += Counter(nx.adjacency_spectrum(graphs.genDivGraph(G.reverse(), ep_dict)))
        else: total_spec += Counter(nx.adjacency_spectrum(graphs.genDivGraph(G, ep_dict)))
...
def ValidateMethod(G,directed=False):
    """runs our eigenvalue catching method and then makes sure that it matches the other method"""
    if directed:
        our_spec = np.array(GetSpectrumFromLEPs(G,directed=directed))
        their_spec = np.array(nx.adjacency_spectrum(G))
    our_spec = np.array(GetSpectrumFromLEPs(G))
    their_spec = np.array(nx.adjacency_spectrum(G))
    # our_spec = np.round(np.array(GetSpectrumFromLEPs(G)), 2)
    # their_spec = np.round(nx.adjacency_spectrum(G), 2)

    print(our_spec)
    print(Counter(their_spec))

    return our_spec == Counter(their_spec)
